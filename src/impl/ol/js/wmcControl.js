...
export class WMCControl extends M.impl.Control {
  /**
   * @classdesc
   * Main constructor of the WMCControl.
   *
   * @constructor
  ...
  getViewContext() {
    ...
    viewContext.General.KeywordList = { Keyword: ["IECA"] };
    ...
    if (maxExtent) {
      viewContext.General.Extension["ol:maxExtent"] = {
        ...
        "$minx": maxExtent.x ? maxExtent.x.min : maxExtent[0],
        "$miny": maxExtent.y ? maxExtent.x.min : maxExtent[1],
        "$maxx": maxExtent.x ? maxExtent.x.min : maxExtent[2],
        "$maxy": maxExtent.y ? maxExtent.x.min : maxExtent[3]
      };
    }
    ...
    for (const layer of this.facadeMap_.getWMS()) {
      ...
      wmcLayer.$hidden = !layer.isVisible() ? 1 : 0;
      wmcLayer.$queryable = layer.isQueryable() ? 1 : 0;
      ...
      wmcLayer.Extension = {
        "ol:opacity": { '#text': layer.getOpacity(), "$xmlns:ol": "http://openlayers.org/context" },
        "ol:transparent": { '#text': layer.transparent, "$xmlns:ol": "http://openlayers.org/context" },
        "ol:isBaseLayer": { '#text': !layer.transparent, "$xmlns:ol": "http://openlayers.org/context" },
        "ol:displayInLayerSwitcher": { '#text': layer.displayInLayerSwitcher, "$xmlns:ol": "http://openlayers.org/context" },
        "ol:singleTile": { '#text': layer.tiled, "$xmlns:ol": "http://openlayers.org/context" }
      };
